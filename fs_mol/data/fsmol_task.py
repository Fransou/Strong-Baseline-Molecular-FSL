...
class MoleculeDatapoint:
    ...
    def get_fingerprint(self) -> np.ndarray:
        if self.fingerprint is not None:
            ...
            fingerprints_vect = rdFingerprintGenerator.GetCountFPs([mol], fpType=rdFingerprintGenerator.MorganFP)[0]
...
class FSMolTask:
    ...
    def load_from_file(path: RichPath, threshold_activity: Optional[float] = None) -> "FSMolTask":
        ...
        for raw_sample in path.read_by_file_suffix():
            all_log_activities.append(float(raw_sample.get("LogRegressionProperty") or 0))
        task_threshold = np.median(all_log_activities)
        for raw_sample in path.read_by_file_suffix():
            ...
            log_activity = float(raw_sample.get("LogRegressionProperty") or 0)
            samples.append(
                MoleculeDatapoint(
