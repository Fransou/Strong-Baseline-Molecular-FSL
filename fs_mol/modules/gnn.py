...
def add_gnn_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--node_embed_dim", type=int, default=128, help="Size of GNN node representations.")
    parser.add_argument(
        "--num_heads",
        type=int,
        default=4,
    ...
    parser.add_argument("--num_gnn_layers", type=int, default=5, help="Number of GNN layers to use.")
    parser.add_argument(
        "--dropout_rate",
        type=float,
        default=0.3,
        help="Dropout rate to use in GNN layers.",
    )
...
class RelationalMultiAggrMP(RelationalMP):
    ...
    def _aggregate_messages(
        ...
        per_node_message_stdev = (
            nn.functional.relu(mean_messages.pow(2) - mean_aggregated_messages[targets].pow(2)) + SMALL_NUMBER
        )
...
class RelationalMultiHeadAttentionMP(nn.Module):
    ...
    def forward(
        ...
        for edge_type, adj_list in enumerate(adj_lists):
            ...
            messages = self.message_fns[edge_type](torch.cat((src_node_reprs, tgt_node_reprs), dim=1))
            ...
            edge_queries = self.target_node_query_projs[edge_type](tgt_node_reprs)  # [E, num_heads * head_dim]
            edge_queries = self.query_scaling_factor * edge_queries
            edge_keys = self.neighbour_node_key_projs[edge_type](src_node_reprs)  # [E, num_heads * head_dim]
...
class GNNBlock(nn.Module):
    ...
    def __init__(self, config: GNNConfig):
        ...
        if config.type.lower() == "MultiHeadAttention".lower():
            ...
            for _ in range(config.num_heads):
                if config.type.lower() == "MultiAggr".lower() or config.type.lower() == "PNA".lower():
                    self.mp_layers.append(
        ...
        self.msg_out_projection = nn.Linear(in_features=total_msg_dim, out_features=config.hidden_dim)
        ...
        if config.intermediate_dim > 0:
            ...
            self.boom_norm_layer: Optional[nn.Module] = nn.LayerNorm(normalized_shape=config.hidden_dim)
        else:
    ...
    def forward(
        ...
        if self.boom_layer is not None and self.boom_norm_layer is not None:
            boomed_representations = self.dropout_layer(self.boom_layer(self.boom_norm_layer(node_representations)))
            if self.config.use_rezero_scaling:
...
class GNN(nn.Module):
    ...
    def forward(self, node_features, adj_lists) -> List[torch.Tensor]:
        ...
        if self.config.make_edges_bidirectional:
            adj_lists = [torch.cat((adj_list, torch.flip(adj_list, dims=(1,))), dim=0) for adj_list in adj_lists]
