...
from fs_mol.configs import GnnPreTrainingConfigLP

from fs_mol.utils.simple_baseline_pretraining_utils import (
    BaseClassPreTrainer,
)
...
def parse_command_line():
    ...
    parser.add_argument(
        "--model_config",
        type=str,
        default="fs_mol/configs/LP/train.json",
...
if __name__ == "__main__":
    ...
    name = (
        name if not name == "" else f"""Lprobe_pretrain_{time.strftime("%d_%b_%H_%M", time.localtime())}"""
    )
    ...
    model_config = GnnPreTrainingConfigLP(**model_config)

    print(model_config)

    fsmol_dataset = FSMolDataset.from_directory(
        directory=RichPath.create(FS_MOL_DATASET_PATH + "/fs-mol"),
        task_list_file=RichPath.create(FS_MOL_DATASET_PATH + "/fsmol-0.json"),
    )
    ...
    trainer = BaseClassPreTrainer(model_config, device, fsmol_dataset)
