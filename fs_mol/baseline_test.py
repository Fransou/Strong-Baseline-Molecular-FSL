...
def run_from_args(args) -> None:
    ...
    wandb.login()
    wandb.init(project="FS-mol")

    datafold = DataFold[args.fold]

    for params in ParameterGrid(DEFAULT_GRID_SEARCH[args.model]):
        ...
        print(results)
        df = pd.DataFrame({k:asdict(v[0]) for k,v in results.items()}).transpose()
        df["delta_aucpr"] = df["avg_precision"] - df["fraction_pos_train"]
        for k,v in params.items():
            df[k] = v
            wandb.log({k: v})
        wandb.log({"delta_aucpr": df.delta_aucpr.mean()})
