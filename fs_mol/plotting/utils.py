...
TRAIN_SIZES_TO_COMPARE = [16,32, 64,128]
...
def summarize_test_run(
    ...
    control_col_strings: str = "num|_frac|seed",
...
def plot_test_run(
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(
        results_df[grouping_column].values, rotation=45, ha="right", fontsize=16
    )
...
def collate_experiment_results(
    ...
    for num in support_set_size:
        try:
            df = pd.read_csv(
                os.path.join(results_dir, f"{model_name}_summary_{x_col}_{num}.csv")
            )
    ...
    if included_frac_cols > 0:
        ...
        merged_df = (
            merged_df.groupby(["TASK_ID"]).mean(numeric_only=True).reset_index().merge(rdf, on="TASK_ID")
        )
...
def _clean_assay(
    ...
    if task_number_prefix is None:
        ...
        df["TASK_ID"] = df.apply(
            lambda x: int(x[task_col][len(task_number_prefix) :]), axis=1
        )
...
def highlight_max_all(
    row, sizes_to_compare: List[int] = [16, 32, 64, 128, 256]
) -> List[str]:
    """
    ...
    for size in sizes_to_compare:
        columns_for_size = [
            col_name for col_name in row.index if col_name.startswith(f"{size}_")
        ]
...
def merge_loaded_dfs(
    ...
    for df in df_list[1:]:
        merged_df = merged_df.merge(
            df,
            how="outer",
            on=["TASK_ID", "fraction_positive_train", "fraction_positive_test"],
        )
...
def plot_task_performances_by_id(
    ...
    for i, model_name in enumerate(model_summaries.keys()):
        ...
        model_auprcs = [
            ...
            for model_result in merged_df[
                f"{support_set_size}_train ({model_name})"
            ].values
        ]
    ...
    frac_pos_to_auprc_ax.set_ylabel(
        f"Average precision with {support_set_size} train points"
    )
    frac_pos_to_auprc_ax.legend()
    frac_pos_to_auprc_ax.set_xlim([0.29, 0.51])
    frac_pos_to_auprc_ax.set_title(
        f"Class imbalance vs. AUPRC: $|T_s|$ = {support_set_size}"
    )
    ...
    if plot_output_dir is not None:
        plt.savefig(
            os.path.join(
                plot_output_dir,
                f"model_comparison_{support_set_size}{highlight_class_str}.png",
            )
        )
        ...
        if len(aggresults) <= 1:  # or aggerrors.notna().sum() <= 1:
            ...
            for model_name in model_summaries.keys():
                if aggresults[f"{num_samples}_train ({model_name})"].notna().sum() == 1:
                    e = pd.Series(
                        data=aggerrors[f"{num_samples}_train ({model_name}) std"]
                        .dropna()
                        .values,
                        index=[f"{num_samples}_train ({model_name}) std"],
                    )
                elif (
                    aggresults[f"{num_samples}_train ({model_name})"].notna().sum() == 0
                ):
                    e = pd.Series(
                        data=float("NaN"),
                        index=[f"{num_samples}_train ({model_name}) std"],
...
def calculate_delta_auprc(
    ...
    for num_samples in train_samples_to_compare:
        for model_name in model_summaries.keys():
            extend_df[
                f"{num_samples}_train ({model_name}) val delta-auprc"
            ] = extend_df.apply(
...
def make_box_plot(
    ...
    if plot_output_dir is not None:
        plt.savefig(
            os.path.join(
                plot_output_dir, f"comparison_boxplot_{support_set_size}_hc_{hc}.png"
            ),
            bbox_inches="tight",
        )
...
def box_plot(
    ...
    if highlight_class is not None:
        extend_df_highlighted = extend_df[
            extend_df["EC_super_class"] == highlight_class
        ]
        make_box_plot(
...
def get_aggregates_across_sizes(
    ...
    for train_size in TRAIN_SIZES_TO_COMPARE:
        aggregation = aggregate_by_class(
            df,
            model_summaries,
            classes=list(df.EC_super_class.unique()),
...
def collect_model_results(
    ...
    for model_name in model_summaries.keys():
        vals[model_name] = df.apply(
            lambda row: grab_row_values(row, model_name), axis=1
        )
        stds[model_name] = df.apply(
            lambda row: grab_row_values_std(row, model_name), axis=1
        )
...
def plot_by_size(
    ...
    categories = {x: i for i, x in enumerate(vals["PN"].index)}
    if highlight_class is not None:
        assert (
    ...
    print(len(model_summaries.keys()))
...
def plot_by_thresh(
    ...
    # pull all values out of the aggregate df
    vals, stds = collect_model_results(df, model_summaries)
    num_labels = pd.read_csv("../dataset/targets_num_labels.csv")
    categories = {x: i for i, x in enumerate(vals["PN"].index)}
    if highlight_class is not None:
        assert (
            str(highlight_class) in categories.keys()
