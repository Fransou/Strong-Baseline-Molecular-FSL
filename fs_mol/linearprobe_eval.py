...
def main():
    ...
    _, dataset = set_up_test_run("linearprobe", args, torch=True)
    ...
    name = (
        args.name
        if not args.name == ""
        else f"""linear_probe_{time.strftime("%d_%b_%H_%M", time.localtime())}"""
    )
    with open(args.model_config, "r") as f:
        model_config = json.load(f)
    model_config = LinearProbeConfig(**model_config)
